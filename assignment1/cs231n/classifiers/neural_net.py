...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    num_train = X.shape[0]
    scores1_raw = np.dot(X, W1) + b1
    scores1 = scores1_raw * (scores1_raw > 0)
    ...
    dW2 = np.dot(scores1.transpose(), scores);
    dW2 /= num_train
    grads['W2'] = dW2 + reg * W2
    grads['b2'] = np.mean(scores, axis=0)

    dscores1 = np.dot(scores, W2.transpose())
    dscores1 /= num_train

    dscores1[scores1_raw < 0] = 0

    dW1 = np.dot(X.transpose(), dscores1)
    grads['W1'] = dW1 + reg * W1

...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(len(hidden_dims)):
            self.params['W' + str(i+1)] = weight_scale * np.random.randn(prev_dim, hidden_dims[i])
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            if (i != self.num_layers - 1):
                if (self.use_batchnorm):
                    out, cache = affine_batchnorm_relu_forward(input, self.params['W' + str(i+1)],
        ...
        for i in range(self.num_layers):
            ...
            if (i != 0):
                ...
                if (self.use_batchnorm):
                    ...
                    dx, dw, db = affine_relu_backward(dx, cache_list[-1-i])
            else:
                dx, dw, db = affine_backward(dx, cache_list[-1-i])
            grads['W' + str(self.num_layers-i)] = self.reg * self.params['W' + str(self.num_layers-i)] + dw
